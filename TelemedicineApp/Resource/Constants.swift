...
struct APIConstant {
    struct BaseURL {
        static let path = "https://thera-api.vercel.app/api/"
    }
    
    struct Endpoints {
        //MARK: - User Auth
        static let doctorSignUp = "users/register/"
        static let appointment = "appointment"
    }
    
    struct Globals {
        static let serverDateFormat = "yyyy-MM-dd'T'HH:mm:ss+00:00"
    }
    
    struct Configurations {
        static let googleAPIKey = "AIzaSyC8pJWaLfqyZA9RclZ_nDiqVu_e--Q9w9c"
    }
}
...
public class API_Almofire
{
    class func RequestAPI(_ apiName: String, type method: String, withparams parameters: [String:Any], isloader : Bool, withController view: UIViewController, sucess success: @escaping (_ response: JSON) -> Void, failure: @escaping (_ error: Error?) -> Void)
    {
        ...
        KRProgressHUD.set(activityIndicatorViewColors: [#colorLiteral(red: 0.2984154224, green: 0.5802860856, blue: 1, alpha: 1)])
...
public class AlertMessage{
    class func displayAlertMessage(message: String) {
