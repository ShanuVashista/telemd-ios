...
extension UpcomingAppoinmentViewController: UITableViewDelegate, UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        print(indexPath.row)
        if self.isfrom == "Upcoming"{
            let vc = storyboard?.instantiateViewController(withIdentifier: "UpcomingAppoinmentDetailsViewController") as! UpcomingAppoinmentDetailsViewController
            self.navigationController?.pushViewController(vc, animated: true)
        }else{
            let vc = storyboard?.instantiateViewController(withIdentifier: "CompletedAppoinmentDetailsViewController") as! CompletedAppoinmentDetailsViewController
            self.navigationController?.pushViewController(vc, animated: true)
        }
